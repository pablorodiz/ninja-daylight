...
util.inherits(sunDriver,stream);
...
var default_latitude = 51.5;
var default_longitude = -0.1;
...
function sunDriver(opts,app) {
  ...
  app.on('client::up',function(){
    ...
    if (!opts.hasSentAnnouncement) {
      ...
      opts.latitude = default_latitude;
      opts.longitude = default_longitude;
      opts.hasSentAnnouncement = true;
      self.save();
    }

    // Register a device
    self.emit('register', new Device(opts));
  });
};
 ...
 */
sunDriver.prototype.config = function(rpc,cb) {
  ...
  if (!rpc) {
    return configHandlers.menu.call(this,this.opts.latitude,this.opts.longitude,cb);
  }
